...
class GCSDataStorageAdapter(BaseDataStorageAdapter):
    ...
    def _get_blob_name(self, symbol: str, freq: CandleUnit) -> str:
        ...
        return f"ohlcv/{symbol}/{unit_file_name_dict[freq]}"
    ...
    def load_ohlcv_data(
        ...
        """
        blob_name = self._get_blob_name(symbol, freq)
    ...
    def delete_ohlcv_data(self, symbol: str, freq: CandleUnit = CANDLE_UNIT) -> None:
        """Delete data from GCS.

        Args:
            symbol: The symbol to delete data for.
            freq: The frequency of the data to delete.
        """
        blob_name = self._get_blob_name(symbol, freq)
        blob = self.bucket.blob(blob_name)
